...
class CAC_Compiler:
	...
	#-----------------------
		self.allleaves = set()
		for no in self.gamma.nodes():
			if len(self.gamma.successors(no))==0:
				self.allleaves.add(no)
	#-----------------------
	...
	# 	allsuccessors = set()
	# 	for s1 in sources:
	# 		allsuccessors |=  set(self.nshape.successors(s1))
	# 		for s2 in sources:
	# 			if s1 != s2:
	# 				locsrc = set()
	# 				locsrc.add(s1)
	...
	# 				successors = set()
	# 				succ_list = []
	# 				successors |=  set(self.nshape.successors(s1))
	# 				successors |=  set(self.nshape.successors(s2))
	# 				succ_list += self.nshape.successors(s1)
	# 				succ_list += self.nshape.successors(s2)
	...
	# 				if len(successors)>0:
	# 					# create the subgraph containing sources and successors nodes
	# 					subgraph_nodes = locsrc | successors
	# 					subgraph = self.nshape.subgraph(list(subgraph_nodes))
	# 					if is_connected :
	# 						# if the graph is connected and is not complete, make it complete, transitive reduction, and recursive call
	# 						if not self.is_complete(len(locsrc),len(successors),subgraph):
	# 							#print "Make complete the subgraph from locsrc"
	# 							self.make_complete(locsrc,successors)

	# 	if len(allsuccessors)>0:
	# 		self.remove_nshape(allsuccessors)
	#-----------------------

	#-----------------------
	# def remove_nshapefromleaves(self,destinations):
	# #-----------------------
	# 	allsources = set()
	# 	for d1 in destinations:
	# 		allsources |=  set(self.nshape.predecessors(d1))
	# 		for d2 in destinations:
	# 			if d1 != d2:
	# 				locdest = set()
	# 				locdest.add(d1)
	# 				locdest.add(d2)
	# 				# compute sources for the given destinations
	# 				# compute a set with single values
	# 				# and compute the list
	# 				sources = set()
	# 				src_list = []
	# 				sources |=  set(self.nshape.predecessors(d1))
	# 				sources |=  set(self.nshape.predecessors(d2))
	# 				src_list += self.nshape.predecessors(d1)
	# 				src_list += self.nshape.predecessors(d2)
				
	# 				# if successors and succ_list has the same size it means that the intersection of successors of sources is empty
	# 				# if successors is shorter than succ_list, the subgraph is connected
	# 				is_connected = (len(sources)<len(src_list))
	...
	# 				if len(sources)>0:
	# 					# create the subgraph containing sources and successors nodes
	# 					subgraph_nodes = locdest | sources
	# 					subgraph = self.nshape.subgraph(list(subgraph_nodes))
	# 					if is_connected :
	# 						# if the graph is connected and is not complete, make it complete, transitive reduction, and recursive call
	# 						if not self.is_complete(len(sources),len(locdest),subgraph):
	# 							#print "Make complete the subgraph from locdest"
	# 							self.make_complete(sources,locdest)

	# 	if len(allsources)>0:
	# 		self.remove_nshapefromleaves(allsources)	
	#-----------------------
	...
	#-----------------------
		...
		saveGraph(self.inverse,"./outputs/inverse2.dot")
