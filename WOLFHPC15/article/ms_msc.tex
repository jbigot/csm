...
Once the synchronized list $\Gamma_{data}$ is obtained, the dependency graph $\Gamma_{hybrid}$ is built and illustrated in Figure~\ref{}. Each computation (and synchronization) is a node of the graph, while each edge represent a read/write data dependency between computations.
...
\begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
  ...
  \path[->]
    ...
    (s2) edge node {} (c3)
         edge node {} (c5);
  \end{tikzpicture}
\caption{$\Gamma_{tsp}$ of Figure~\ref{fig:msinput}.}
\label{fig:canon}
\end{center}
\end{figure}
