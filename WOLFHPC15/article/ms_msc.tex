...
\paragraph{Computations description} The most important part of MSL is the description of the simulation computations. The language follows the three definitions given in Section~\ref{sect:multistencil}, however as MSL is not made to express numerical computations, the element $exp$ of definitions~\ref{eq:st} and~\ref{eq:loc} is not given in the language. Moreover, to propose a syntax close to an imperative code, information are given in a different order.
