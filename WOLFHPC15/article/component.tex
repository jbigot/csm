...
%\subsection{Overview}
%----------------------------------------
This section describes the projection of a MSP program to a component-based runtime, including the components used to implement the static scheduling of $\Gamma_{tsp}$.
...
This is represented on Figure~\ref{fig:mscac:assembly} where each rounded box represents one component instance, drawn with simple lines, or more, drawn with double lines. For example, the component \texttt{Data} is instanciated more than once.
...
As presented on Figure~\ref{fig:k}, a $K$ component provides a single port used to launch the computation and exposes one use port for each data element manipulated in the computation.
We denote the fact that it may have several use port by adding a star on the use port to \texttt{Data}.
...
Thus, this figure represents what the component \texttt{Computations} of Figure~\ref{fig:mscac:assembly} contains if an hybrid parallelization is performed. On the other hand, if a data parallelization is performed, a single $SEQ$ component is used by \texttt{Time}, which itself uses in the appropriate order the computation kernels and synchronizations. Among those kernels, a single one \emph{fusionned} represents $c_3$ and $c_5$.
