...
\begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
   ...
   \node[use,below=0.8cm of T] (Tu) {};
   \node[left=0.8cm of T] (tt) {$T$};

   \node[provide,below=0.15 of Tu] (Sp) {};
   \node[component,below=0.8cm of Sp] (S) {$Scheduling$};
   \node[use,right=1.5cm of S] (Su) {};
   \node[use,right=1.5cm of Tu] (Su2) {};
   \node[left=1.5cm of S] (gamma) {$\Gamma_{tsp}$};

   \node[provide,right=0.15 of Su] (Cp) {};
   \node[component,right=1.5cm of Cp] (C) {$Kernels$};
   ...
   \node[provide,right=0.15 of Su2] (Datap3) {};
...
\texttt{Driver} is static and drives the whole application while \texttt{Kernels} contains code directly written by the user.
The four other parts are generated based on the content of sections of the MSL program: \texttt{Scheduling} from $\Gamma$, \emph{DDS} from $\mathcal{M}$ and $\mathcal{E}$, \emph{Data} from $\Delta$ and $\mathcal{D}$, and \emph{Time} from $T$.
...
\texttt{Scheduling} contains the static scheduling computed by MSC, \ie $\Gamma_{tsp}$.
...
At the leafs of the tree, the components instances make call either to \texttt{Data} for synchronization of to \texttt{Kernels} for computation.

\texttt{Kernels} contains code written by the user to implement the actual computation.
...
Similarly, changing the approach used for scheduling would only impact the \texttt{Scheduling} components.
...
In this case however, the \texttt{Kernels} components written by the user have to be adapted to the new interface, but this is the only impact.
