The solution proposed in this paper is based on both a domain specific language to describe a multi-stencil program, \ie a complete numerical simulation, and on a compiler which parses the language, applies transformations for automatic parallelization, and dumps the representation to a component-based runtime.
...
\begin{tikzpicture}[remember picture,
  ...
  \node (MSL) {MSL};
  \node[outer,right=0.5cm of MSL,draw=black] (MSCAC) {
    \begin{tikzpicture}
      \node [inner,draw=black,thick] (pars)  {Parser};
      \node [inner,draw=black,thick,right=0.5cm of pars] (MSC)  {MSC};
      \node [inner,right=0.5cm of MSC,draw=black,thick] (Dump)  {Dump};

      \draw[->] (pars) -- (MSC);
      \draw[->] (MSC) -- (Dump);
    \end{tikzpicture}
  };
  \node [right=0.5cm of MSCAC] (CA) {CBR};
  \node [above =0.2cm of MSCAC] (CAid) {MSCAC};
  \draw[->] (MSL) -- (pars.west);
  \draw[->] (Dump.east) -- (CA);
\end{tikzpicture}
\caption{The three phases of the MSCAC compiler: \emph{i)} the parser of the MSL description; \emph{ii)} the MSC parallelization; \emph{iii)} the dump to the Component-Based Runtime (CBR).\label{fig:mscac}}
\end{center}
\end{figure}

As illustrated in Figure~\ref{fig:mscac}, the compiler, named MSCAC for \emph{Multi-Stencil Component Assembly Compiler}, is composed of three different phases. The first phase \emph{parses} the MSL description into an intermediate representation (IR). The second phase called MSC transforms the computation part ($\Gamma$) of the IR into a series-parallel tree decomposition~\cite{Valdes:1979:RSP:800135.804393} to introduce parallelism in the overall numerical simulation.
...
Different strategies can be used to schedule the tasks of this graph, \ie to plan their execution in a valid order. The MSC compiler presented in this paper handles the transformation of the dependency graph to a static scheduling.
...
These properties counter some known limitations of DSLs. % JB: was "parallel applications".
...
Those are the components implementing: \emph{a)} the computations scheduling ($\Gamma$), \textbf{b)} the data structures ($\mathcal{M}$ and $\mathcal{E}$), \textbf{c)} data of the simulation ($\Delta$ and $\mathcal{D}$), and \textbf{d)} the time loop ($T$). The actual implementation of the numerical expressions are provided by the user and used in the computations scheduling.
This component-based parallel structure enables to change the technology used for each part while limiting the impact on the other parts, as discussed in Section~\ref{sect:component}.
