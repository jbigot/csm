...
plt.loglog(cores, itsMSL, 'b-o', label='MSL data parallelization only', basex=2,basey=2)
plt.loglog(cores, itsMSLfus, 'b-x', label='MSL data parallelization only', basex=2,basey=2)
plt.loglog(cores, itsT2, 'r-*', label='2 cores for tasks without fusion', basex=2,basey=2)
#plt.loglog(cores, itsT8, 'r-*', label='8 cores for tasks without fusion', basex=2,basey=2)
plt.loglog(cores, itsT2fus, 'r-+', label='2 cores for tasks with fusion', basex=2,basey=2)
#plt.loglog(cores, itsT8fus, 'r-+', label='8 cores for tasks with fusions', basex=2,basey=2)

legend = plt.legend(loc='lower right', shadow=True,prop={'size':12})
plt.grid(True, which="both", linestyle="dotted")
plt.xlabel('cores')
plt.ylabel('iterations per second')
pp = PdfPages('withwithout2_close_median.pdf')
