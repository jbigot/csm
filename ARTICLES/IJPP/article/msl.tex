...
comp ::= quantityid "[" compdomid "]" "=" compid "(" listread ")"
...
\item The \texttt{mesh} keyword (Fig.\ref{fig:grammar},~l.1) introduces a identifier for $\mathcal{M}$, the single mesh of the simulation. For example \texttt{cart} in Fig.\ref{fig:exmsl},~l.1. Since the language is independent of the mesh topology, this is not used by the compiler and present for reference only. 

\item The \texttt{mesh entities} keyword (Fig.\ref{fig:grammar},~l.2) introduces identifiers for the groups of entities $\mathcal{G}\in\Phi$. For example \texttt{cell} or \texttt{edgex} in Fig.\ref{fig:exmsl},~l.2. Again, this is for reference only as the compiler does not rely on the mesh topology.

\item The \texttt{computation domains} keyword (Fig.\ref{fig:grammar},~l.3) introduces identifiers for the computation domains $D\in\mathcal{D}$. For example \texttt{d1} and \texttt{d2} in Fig.\ref{fig:exmsl},~l.4-5. For reference, each domain is associated to a group of entities (Fig.\ref{fig:grammar},~l.15) such as \texttt{cell} for \texttt{d1} in Fig.\ref{fig:exmsl},~l.4.

\item The \texttt{independent} keyword (Fig.\ref{fig:grammar},~l.5) offers a way to declare that computation domains do not overlap Such as \texttt{d1} and \texttt{d2} in  Fig.\ref{fig:exmsl},~l.7. This is used by the compiler to compute dependencies between computations.

\item The \texttt{stencil} keyword (Fig.\ref{fig:grammar},~l.7) introduces identifiers for $n\in\mathcal{N}$, the stencil shapes. For each shape, the source and destination group of entities (Fig.\ref{fig:grammar},~l.19) are specified. For example \texttt{nec} in Fig.\ref{fig:exmsl},~l.11 associates \texttt{cell} entities to each \texttt{edgex} entity.

\item The \texttt{quantities} keyword (Fig.\ref{fig:grammar},~l.9) introduces identifiers for $\delta\in\Delta$, the quantities with the group of entities they are mapped on (Fig.\ref{fig:grammar},~l.21). For example the quantities \texttt{C} and \texttt{H} are mapped on \texttt{edgex} entities.

\item The \texttt{scalar} keyword (Fig.\ref{fig:grammar},~l.10) introduces identifiers for $s\in\mathcal{S}$, the scalars. For example \texttt{mu} and \texttt{tau} in Fig.\ref{fig:exmsl},~l.15. 

\item Finally, the last part (Fig.\ref{fig:grammar},~l.11) introduce the computations steps. Each step is made of two parts:
\begin{itemize}
\item the \texttt{time} keyword introduces $conv$, the convergence criteria either as a number of iterations (Fig.\ref{fig:grammar},~l.27) or as a function of scalars (Fig.\ref{fig:grammar},~l.28). 500 iterations are for example specified in Fig.\ref{fig:exmsl},~l.16,
\item the \texttt{computations} keyword (Fig.\ref{fig:grammar},~l.26) introduces identifiers for the computation $k\in\Gamma=(S,R,(w,D),comp)$.
\end{itemize}

\item each computation (Fig.\ref{fig:grammar},~l.31) specifies:
\begin{itemize}
...
One can notice that in the example of Figure~\ref{fig:exmsl}, there are no (reduction) kernel associated to the scalars.
In this case, the scalars are in fact constants.
One can also notice that the computation to execute for each kernel is not specified.
This is indeed not handled by MSL that only generates a skeleton of the application.
It can then be specified elsewhere by referencing the identifier chosen here.
...
The next section will show how parallelization can be extracted from this simple language and how an empty parallel skeleton of the application can be generated. %, introducing implementation concerns in a second phase.
