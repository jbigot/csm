...
%\begin{mydef}
\begin{itemize}
\item \textit{A quantity (or data) is a function $\delta: E_{\delta} \mapsto V_{\delta}$ which associates each entity of a group $e\in E_{\delta}$ to a value $v\in V_{\delta}$ where $V_{\delta}$ is a data type, typically $\mathbb{R}$, $\mathbb{N}$ or $\mathbb{C}$.
...
%\begin{mydef}
\begin{itemize}
\item A computation domain $D$ is a subpart  a mesh entities group, $D \subset E \in \mathcal{E}$.
...
%\begin{mydef}
\begin{itemize}
\item A reduction computation $r$ of a numerical simulation is defined as $r(R,w,red)$, where $R$ is the set of data read, $w \in \mathcal{S}$ is the scalar written, and $\text{red}$ is an associative reduction function.
