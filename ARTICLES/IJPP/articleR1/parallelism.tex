As previously explained, in a computation $k(S,R,(w,D),comp)$, $comp$ is not handled by MSF. Numerical codes are left in a second step to the developer by using the $DDS$ component selected during the MSF compilation. As a result, in the rest of this paper, and to simplify notations, we denote the same computation $k(S,R,(w,D))$.
...
Actually, a synchronization is needed by the quantity read by a stencil computation (not local), if this quantity has been written before. This synchronization is needed because a neighborhood function $n \in \mathcal{N}$ of a stencil computation involves values computed on different resources.
...
We are not arguing that such a simple fusion algorithm could be as good as complex cache optimization techniques which can be found in stencil DSLs for example~\cite{spaaTangCKLL11}. However, this fusion takes place at a different level and can bring performance improvements as it will be illustrated in Section~\ref{sect:eval}. This fusion algorithm relies on two statements:
\begin{itemize}
...
To illustrate how a specialized assembly is generated, the specialized assembly of the example that has been used throughout this paper is represented by Fig.~\ref{specass}. As this assembly is heavy to entirely draw, only a sub part of it is represented by the figure. Moreover some connections are represented by numbers instead of lines to not complexity the read.
...
\begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
   ...
   \node[component, fill=green!80!black,right=2cm of Cp] (C) {$Scheduler$};
   \node[use,right=1.8 of C] (scu1) {};
   \node[use,below=0.2 of scu1] (scu2) {};

   \node[provide,below right=0.2 of Du3] (Datap1) {};
   \node[component,draw=red,fill=red!50!white,above=0.8cm of Datap2] (Data) {$A$};
   \node[component,draw=red,fill=red!50!white,below=0.8cm of Data] (Data2) {$B$};
   ...
   \node[component, fill=blue!60!white,right=2cm of pk0] (k0) {$k_0$};
   \node[use,right=0.8 of k0] (k0u) {};
   \node[right=0.2 of k0u] (1) {1};
   \node[use,below=0.2 of k0u] (k0u2) {};
   \node[right=0.2 of k0u2] (1) {2};
   \node[component, fill=blue!60!white,below=0.8cm of k0] (sync1) {$Sync(0,1)$};
