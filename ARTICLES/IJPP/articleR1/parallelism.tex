In a computation $k(S,R,(w,D),comp)$, the $comp$ part is provided by a developer after the MSC compilation phase.
...
\label{def:sync}
\CP{rappeler ce que sont $w_i$ et $r_j$ ?? Je les ai ajouté ci-dessous mais cela ne me semble pas suffisant. La def n'est pas self-contained.}
\end{mydef}

A synchronization is needed for the quantity ($r_j$) read by a stencil computation (not local), if this quantity has been written before ($w_i$). This synchronization is needed because a neighborhood function $n \in \mathcal{N}$ of a stencil computation involves values computed on different resources.
...
\CP{C'est quoi les 2 types de sync ?}
...
\paragraph{\textbf{Example}} Figure~\ref{fig:exmsl}~\vpageref{fig:exmsl} gives an example of a $\mathcal{MSP}$ program. From this example, the following ordered list of computation kernels can be extracted:
\begin{equation*}
...
\CP{Phrase précédente pas claire}
%
...
\begin{mydef}
From the synchronized ordered list of computation kernels $\Gamma_{sync}$, the dependency graph of the computations $\Gamma_{dep}(V,A)$ is obtained from the call $F_{dep}(\Gamma_{sync},0)$, where $F_{dep}$ is the recursive function defined in Algorithm~\ref{alg:dep}~\vpageref{alg:dep}.
\end{mydef}
...
\paragraph{\textbf{Example}} From $\Gamma_{dep}$ illustrated in Figure~\ref{fig:depdep}~on page~\pageref{fig:depdep}, the TSP tree represented in Figure~\ref{fig:tree}~\vpageref{fig:tree} can be computed.
...
\CP{Pour moi $T_{COM}$ augmente avec P s'il y a au moins une op\'erations collectives (reduction/barrière) dont le cout est en log(P); Of course, s'il n'y a que des comm entre voisins, $T_{COM}$ tend vers la latence du réseau (quand il n'y a plus qu'une donnée par voisin)}
