...
For simulations, data is discretized based on the mesh; but for a given mesh, there are various ways of mapping the data depending on the mesh entities on which the data is applied.
% \begin{mydef}
...
\begin{mydef}
A computation kernel $k$ of a numerical simulation is defined as $k(R,w,\text{exp})$, where $R \subset \Delta, w \in \Delta$ and $\text{exp}$ a numerical expression. %$D$ is one of the subsets $D_i \subset \mathcal{M}$, such that $w : D \rightarrow V$.
\end{mydef}
