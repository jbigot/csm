...
From an ordered list of computations $\Gamma_{sync}$, a directed dependency graph $\Gamma_{dep}(V,A)$ can be built finding all pairs of computations $c_i(R_i,w_i,\text{exp}_i,D_i)$ and $c_j(R_j,w_j,\text{exp}_j,D_j)$, with $i<j$, such that $c_i \prec c_j$. %Those time dependencies can be found from pairs of data dependencies $c_i \prec c_j$.
...
This constructive function is possible because the input is an ordered list. Actually, if $c_i\prec c_j$ then $i<j$. As a result, $c_i$ is already in $V$ when the arc $(\overset{\frown}{c_i,c_j})$ is built. 
