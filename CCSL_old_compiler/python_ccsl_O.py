...
#This version try a pseudo overlapping version of components TimeO and PhaseO
#################################################################
...
def createAssembly(meshWidth,meshHeight,nprocs,
    ...
    for i in range(nprocs):
        ...
        #object Time
        time= makeInstance(process, 'ti_'+str(i), 'TimeO')
        ...
        for phase in phgpk:
            ...
            ph = makeInstance(process, 'ph_'+str(cph)+'_'+str(i), 'PhaseO')
            #add cppref to Time component
            timephase.append(('inGo', 'ph_'+str(cph)+'_'+str(i)))
            timephaseO.append(('services', 'ph_'+str(cph)+'_'+str(i)))
            #write output file
        ...
        timephaseO.sort()
        makeConnection(time, 'phaseInit', timephaseO)
...
#TODO--Phase 4: Generation of a HLCM composite
